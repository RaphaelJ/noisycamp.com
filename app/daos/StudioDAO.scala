...
class StudioDAO @Inject()
    ...
    final class StudioTable(tag: Tag) extends Table[Studio](tag, "studio") {
        ...
        def useLive             = column[Boolean]("use_live")
        def useLessons          = column[Boolean]("use_lessons")
        ...
        private type OpeningTimesTuple = (Boolean, Option[LocalTime], Option[LocalTime])

        private type PricingPolicyTuple = (
            BigDecimal, Boolean, Option[LocalTime], Option[BigDecimal],
            Boolean, Option[BigDecimal])

        private type BookingPolicyTuple = (Duration, Boolean, Boolean, Option[Duration])
        ...
        private def toLocation(locationTuple: LocationTuple): Location = {
            Location(Address.tupled(locationTuple._1), Coordinates.tupled(locationTuple._2))
        }

        private def fromLocation(location: Location): LocationTuple = {
            (Address.unapply(location.address).get, Coordinates.unapply(location.coordinates).get)
        }

        private def toOpeningSchedule(scheduleTuple: OpeningScheduleTuple) : OpeningSchedule = {
        ...
        private def toBookingPolicy(policyTuple: BookingPolicyTuple) = {
            BookingPolicy(policyTuple._1, policyTuple._2, policyTuple._4.map(CancellationPolicy))
        }

        private def fromBookingPolicy(policy: BookingPolicy) = {
            (policy.minBookingDuration, policy.automaticApproval,
                policy.cancellationPolicy.isDefined, policy.cancellationPolicy.map(_.notice))
        }
    ...
    def publishedStudios = query.filter(_.published)
